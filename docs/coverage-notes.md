...
| 维度 | 实测（c8b3704） | 门禁（`--fail-under-*`） |
|---|---|---|
| 行 | 90.5%（1274/1407） | 90.1%（容 ~6 行抖动） |
| 分支 | 77.9%（950/1219） | 77.4%（容 ~6 分支抖动） |

分文件行覆盖：`ffmpeg_backend.cpp` 92%（1001/1088）、`null_backend.cpp` 100%、`player.cpp` 100%、`main.cpp` 67%（SDL 窗口交互块）。

分文件分支覆盖：`ffmpeg_backend.cpp` 81%（722/891）、`null_backend.cpp` 79%、`player.cpp` 90%、`main.cpp` 56%。
...
行 90.5% 与分支 77.9% 是**当前代码库在“不删防御代码、不写假用例、不做进程污染”前提下的真实上限**。凑到字面 100% 只能：删掉防御分支、mock 掉被测库、或写不断言的假用例——三者都违背项目铁律。新增可测路径时按既有模式补测（真实文件、真实失败契约），并把门禁阈值随实测水位上抬。
