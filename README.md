...
项目处于 MVP（v0.1）早期阶段：核心播放链路（打开 → 解码 → 音视频同步 → 渲染/出声）已经在 FFmpeg + SDL2 后端上跑通，CLI 冒烟入口和 SDL2 窗口可用。轨道运行时切换、字幕渲染、播放列表等仍在路线图上（见下文 roadmap），欢迎按 [docs/mvp.md](docs/mvp.md) 的边界一起推进。
...
- **轨道选择**：音频轨切换（当前限停止状态）、字幕轨选择记录（字幕渲染尚未实现）；
